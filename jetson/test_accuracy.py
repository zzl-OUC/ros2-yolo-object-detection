...
# -*- coding: utf-8 -*-
"""
20 物体识别率测试 (验收要求: 测试 20 个物体, 正确识别率 >= 80%)

功能对应验收点:
  1. 测试 20 个物体并统计正确识别率
  2. 保存测试结果 (results.csv + summary.txt + annotated/ 标注图)
  3. 保存典型错误案例 (errors/ 目录, 自动收集识别错的样本)
  4. 顺带输出单帧推理耗时与折算 FPS, 辅助核对 ">=5 FPS" 指标

运行方式 (Jetson 上):
  1. 拍 20 张测试照片放进 test_images/, 文件名以真实类别开头, 例如:
       mouse_01.jpg  mouse_02.jpg ... phone_01.jpg ...
     (类别名须与模型类别一致: mouse / phone;
...
def build_cli():
    ...
    p.add_argument("--model", default=None, help="权重路径; 缺省时按 best.engine / best.pt 顺序找")
...
def guess_label(img_path, gt, names):
    """从文件名/CSV 推断期望类别; 推断不出返回 None。"""
    base = os.path.basename(img_path)
    if img_path in gt or base in gt:
        return gt.get(img_path) or gt.get(base)
...
def main():
    args = build_cli()
    net = YOLO(locate_weights(args.model))
    try:
        classes = net.names
        ...
        for cand in (os.path.join(here, "best.pt"), "best.pt"):
            if os.path.exists(cand):
                classes = YOLO(cand).names
                break
        else:
            raise SystemExit("无法获取类别名: 请把 best.pt 放到脚本同目录后再跑")

    samples = sorted(os.path.join(args.images, f) for f in os.listdir(args.images)
    ...
    for img_path in samples:
        expected = guess_label(img_path, gt, classes)
        if expected is None:
            unknown.append(os.path.basename(img_path))
            continue

        img = cv2.imread(img_path)
        t0 = time.perf_counter()
        result = net.predict(img, imgsz=args.imgsz, conf=args.conf, verbose=False)[0]
        ...
        if len(boxes):
            ...
            pred, conf = classes[pred_id], float(boxes.conf[i])
        else:
            pred, conf = "(未检出)", 0.0
        hit = pred == expected
        ...
        records.append({"文件名": os.path.basename(img_path), "期望类别": expected,
                        "识别类别": pred, "置信度": f"{conf:.3f}",
                        "推理耗时ms": f"{ms:.0f}", "是否正确": "是" if hit else "否"})
        if not hit:  # 典型错误案例: 识别错/未检出的留档
            shutil.copy(annotated_path, os.path.join(dirs["errors"], os.path.basename(img_path)))
        print(f"[{'OK ' if hit else 'ERR'}] {os.path.basename(img_path):24s} "
    ...
    total = len(records)
    correct_n = sum(1 for r in records if r["是否正确"] == "是")
    acc = correct_n / total * 100 if total else 0.0
    avg_ms = sum(float(r["推理耗时ms"]) for r in records) / total if total else 0.0
